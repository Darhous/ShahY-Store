...
export default function BannersCarousel({ banners }: { banners: Banner[] }) {
  ...
  const Wrapper = current.link
    ? ({ children }: { children: React.ReactNode }) => (
        <Link href={current.link!} style={{ display: "block", textDecoration: "none" }}>{children}</Link>
      )
    : ({ children }: { children: React.ReactNode }) => <div>{children}</div>

  return (
    <div
      style={{ position: "relative", width: "100%", overflow: "hidden", background: "#0E0C09" }}
      onMouseEnter={() => setPaused(true)}
      onMouseLeave={() => setPaused(false)}
    >
      <style>{`
        @keyframes bannerFade { from{opacity:0;transform:scale(1.02)} to{opacity:1;transform:scale(1)} }
      `}</style>
